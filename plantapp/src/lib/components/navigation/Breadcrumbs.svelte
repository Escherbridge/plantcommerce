<script lang="ts">
	...
	function processItems(): BreadcrumbItem[] {
		...
		if (maxItems && processedList.length > maxItems) {
			...
			processedList = [firstItem, { name: '...', isActive: false }, ...lastItems];
		}
	...
	<ol class="flex items-center space-x-2 text-sm">
		{#each processedItems as item, index}
			<li class="flex items-center">
				...
				{#if item.name === '...'}
					<!-- Ellipsis item -->
					<span class="px-2 text-base-content/40" aria-label="More breadcrumbs"> ... </span>
				{:else if item.isActive || !item.url}
					<!-- Active/current item -->
					<span class="font-medium text-base-content {activeClassName}" aria-current="page">
						{item.name}
					</span>
				{:else}
					<!-- Linked item -->
					<a
						href={item.url}
						class="text-base-content/70 transition-colors duration-200 hover:text-base-content {linkClassName}"
