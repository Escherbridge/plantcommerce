...
{#if lightboxOpen && showLightbox}
	...
	>
		<!-- Close button -->
		<button
			class="btn absolute top-4 right-4 z-10 btn-circle text-white btn-ghost hover:bg-white/20"
			onclick={closeLightbox}
		...
		{#if images.length > 1}
			<button
				class="btn absolute top-1/2 left-4 btn-circle -translate-y-1/2 text-white btn-ghost hover:bg-white/20"
				onclick={(e) => {
					e.stopPropagation();
					goToPrevious();
				}}
		...
		{#if images.length > 1}
			<button
				class="btn absolute top-1/2 right-4 btn-circle -translate-y-1/2 text-white btn-ghost hover:bg-white/20"
				onclick={(e) => {
					e.stopPropagation();
					goToNext();
				}}
