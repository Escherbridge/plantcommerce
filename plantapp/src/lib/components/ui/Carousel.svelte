...
>
	<div
		class="rounded focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:outline-none"
		role="group"
	>
		<!-- Main carousel track -->
		<div
		...
		{#if showArrows && items.length > currentSlidesToShow}
			<!-- Previous arrow -->
			<button
				class="carousel-arrow carousel-arrow-prev btn absolute top-1/2 left-4 z-10 btn-circle -translate-y-1/2 border-none bg-black/20 text-white btn-ghost hover:bg-black/40"
				onclick={() => {
					goToPrevious();
			...
			<!-- Next arrow -->
			<button
				class="carousel-arrow carousel-arrow-next btn absolute top-1/2 right-4 z-10 btn-circle -translate-y-1/2 border-none bg-black/20 text-white btn-ghost hover:bg-black/40"
				onclick={() => {
					goToNext();
					handleUserInteraction();
				}}
		...
		{#if showDots && items.length > currentSlidesToShow}
			<div class="carousel-dots absolute bottom-4 left-1/2 z-10 flex -translate-x-1/2 gap-2">
				{#each Array(Math.ceil(items.length / currentSlidesToShow)) as _, dotIndex}
					<button
						class="carousel-dot h-3 w-3 rounded-full transition-all duration-200
