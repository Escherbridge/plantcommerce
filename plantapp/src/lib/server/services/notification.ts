...
export class NotificationService {
  static async create(userId: string, type: string, title: string, message: string, link?: string) {
    return db.insert(table.notification).values({ userId, type: type as table.Notification['type'], title, message, link }).returning();
  }

  static async getUnread(userId: string) {
    return db.select().from(table.notification)
  ...
  static async markRead(id: string, userId: string) {
    return db.update(table.notification)
      .set({ isRead: true })
  ...
  static async getUnreadCount(userId: string): Promise<number> {
    const result = await db.select().from(table.notification)
      .where(and(eq(table.notification.userId, userId), eq(table.notification.isRead, false)));
