...
export class StripeCheckoutService {
	...
	static async createCheckoutSession(params: CreateCheckoutParams): Promise<Stripe.Checkout.Session> {
		...
		if (!Number.isFinite(expiresAtSeconds) || expiresAtSeconds * 1000 - Date.now() < 30 * 60 * 1000) {
			throw new Error('Stripe Checkout sessions require at least 30 minutes of remaining draft lifetime');
		}
		for (const item of lineItems) {
			if (!Number.isSafeInteger(item.unitPriceMinor) || item.unitPriceMinor < 0
				|| !Number.isSafeInteger(item.quantity) || item.quantity <= 0) {
				throw new Error('Checkout line items must use positive safe-integer quantities and minor-unit prices');
			}
		}
