...
The public catalog RPC now returns no product, category, price, inventory, image, or structured-data payload until a reviewed ingestion path exists. The `/products` route is explicitly noindex and reports verification in progress; legacy category/detail URLs redirect or return an observable unavailable response. Cart reads/additions, checkout-draft creation, affiliate link creation, public affiliate lookup, and affiliate redirects use the same guard so a product ID cannot bypass this boundary. The following surfaces remain relevant until their claims are backed by reviewed operational facts:
...
| Product availability and structured data | The public router in `plantapp/src/lib/server/api/products.ts` is closed by `catalogTruth/publicCatalog.ts`; direct detail routes fail before product data or structured data render. | Keep it closed until catalog truth and inventory verification exist.                      |
