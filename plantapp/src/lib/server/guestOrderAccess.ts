...
export class GuestOrderAccessService {
	...
	static async issueForDraft(draftId: string, now = new Date()): Promise<GuestOrderAccessGrant> {
		return await db.transaction(async (tx) => {
			...
			if (!draft || draft.userId !== null || !draft.guestSubjectHash) {
				throw new Error('Guest order confirmation access is only available for a guest checkout draft');
			}
	...
	static async hasDraftAccess(cookies: Cookies, draftId: string, now = new Date()): Promise<boolean> {
		...
		const [grant] = await db
			...
			.innerJoin(table.checkoutDraft, eq(table.guestOrderAccessGrant.draftId, table.checkoutDraft.id))
			.where(and(
