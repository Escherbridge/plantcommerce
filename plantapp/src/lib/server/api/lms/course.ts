...
export const lmsCourseRouter = router({
	...
	create: protectedProcedure
		.input(
			z.object({
				...
				courseType: z
					.enum(['self_paced', 'instructor_led', 'blended', 'cohort'])
					.default('self_paced'),
		...
		.mutation(async ({ ctx, input }) => {
			LmsAccessService.requireCourseCreator(ctx.user);
			// If instructor, force instructorId to their own ID
			const params =
				ctx.user.role === 'instructor' ? { ...input, instructorId: ctx.user.id } : input;
			const course = await LmsCourseService.createCourse(params);
			await AuditLogService.log(ctx.user.id, 'lms_create_course', {
				courseId: course.id,
				title: course.title
			});
			return course;
	...
	update: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			const { id, instructorId, ...params } = input;
			await LmsAccessService.requireCourseManager(ctx.user, id);
			const course = await LmsCourseService.updateCourse(
				id,
				ctx.user.role === 'admin' && instructorId !== undefined
					? { ...params, instructorId }
					: params
			);
	...
	clone: adminProcedure
		...
		.mutation(async ({ ctx, input }) => {
			const course = await LmsCourseService.cloneCourse(
				input.courseId,
				input.newTitle,
				input.newSlug
			);
			await AuditLogService.log(ctx.user.id, 'lms_clone_course', {
	...
	publish: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			await LmsAccessService.requireCourseManager(ctx.user, input.id);
