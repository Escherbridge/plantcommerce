import { pgTable, serial, integer, bigint, text, timestamp, decimal, boolean, uuid, varchar, index, uniqueIndex, check, foreignKey } from 'drizzle-orm/pg-core';
...
export const emailChangeCapability = pgTable('email_change_capability', {
	userId: text('user_id')
		.primaryKey()
		.references(() => user.id),
	...
	previousEmailConfirmedAt: timestamp('previous_email_confirmed_at', { withTimezone: true, mode: 'date' }),
	expiresAt: timestamp('expires_at', { withTimezone: true, mode: 'date' }).notNull(),
	createdAt: timestamp('created_at', { withTimezone: true, mode: 'date' }).notNull().defaultNow()
}, (table) => ({
	newEmailTokenIdx: uniqueIndex('email_change_capability_new_token_idx').on(table.newEmailTokenHash),
	previousEmailTokenIdx: uniqueIndex('email_change_capability_previous_token_idx').on(table.previousEmailTokenHash),
...
export const loginAttempts = pgTable('login_attempts', {
	...
	identifierIdx: uniqueIndex('login_attempts_identifier_idx').on(table.identifier, table.identifierType),
...
export const affiliateAttribution = pgTable('affiliate_attribution', {
	id: text('id').primaryKey(),
	...
	capabilityHashIdx: uniqueIndex('affiliate_attribution_capability_hash_idx').on(table.capabilityHash),
	linkIdx: index('affiliate_attribution_link_idx').on(table.affiliateLinkId),
	clientExpiresIdx: index('affiliate_attribution_client_expires_idx').on(table.clientHash, table.expiresAt),
...
export const affiliateAttributionClick = pgTable('affiliate_attribution_click', {
	id: serial('id').primaryKey(),
	affiliateAttributionId: text('affiliate_attribution_id').references(() => affiliateAttribution.id, {
		onDelete: 'set null'
	}),
	affiliateLinkId: integer('affiliate_link_id')
		.notNull()
	...
	linkDateIdx: index('affiliate_attribution_click_link_date_idx').on(table.affiliateLinkId, table.clickedAt),
	clientDateIdx: index('affiliate_attribution_click_client_date_idx').on(table.clientHash, table.clickedAt),
	attributionIdx: index('affiliate_attribution_click_attribution_idx').on(table.affiliateAttributionId)
}));

// A single row per link/client makes rolling-window click deduplication atomic.
export const affiliateClickDedupe = pgTable('affiliate_click_dedupe', {
	id: serial('id').primaryKey(),
	affiliateLinkId: integer('affiliate_link_id')
		.notNull()
		.references(() => affiliateLink.id, { onDelete: 'cascade' }),
	...
	linkClientIdx: uniqueIndex('affiliate_click_dedupe_link_client_idx').on(table.affiliateLinkId, table.clientHash),
...
export const affiliateTier = pgTable('affiliate_tier', {
	id: text('id').primaryKey(),
	...
	rateRange: check('affiliate_tier_rate_bps_check', sql`${table.commissionRateBps} >= 0 AND ${table.commissionRateBps} <= 10000`)
}));

// Acceptance records are append-only evidence for the terms/disclosure version used by a ledger entry.
export const affiliateTermsAcceptance = pgTable('affiliate_terms_acceptance', {
	id: text('id').primaryKey(),
	affiliateId: integer('affiliate_id')
		.notNull()
	...
	acceptanceReferenceIdx: uniqueIndex('affiliate_terms_acceptance_reference_idx').on(table.acceptanceReference),
	affiliateVersionIdx: uniqueIndex('affiliate_terms_acceptance_version_idx').on(
		table.affiliateId,
		table.termsVersion,
		table.disclosureVersion
	),
	affiliateAcceptedIdx: index('affiliate_terms_acceptance_affiliate_accepted_idx').on(table.affiliateId, table.acceptedAt)
}));
...
export const affiliatePayout = pgTable('affiliate_payout', {
	...
	affiliateCreatedIdx: index('affiliate_payout_affiliate_created_idx').on(table.affiliateId, table.createdAt),
	amountPositive: check('affiliate_payout_amount_check', sql`${table.amountMinor} > 0 AND ${table.amountMinor} <= 9007199254740991`),
	currencyFormat: check('affiliate_payout_currency_check', sql`${table.currency} ~ '^[a-z]{3}$'`),
	periodOrder: check('affiliate_payout_period_check', sql`${table.periodStart} IS NULL OR ${table.periodEnd} IS NULL OR ${table.periodStart} <= ${table.periodEnd}`)
}));
...
export const cart = pgTable('cart', {
	...
	userIdentityIdx: uniqueIndex('cart_user_id_unique').on(table.userId).where(sql`${table.userId} IS NOT NULL`),
	guestIdentityIdx: uniqueIndex('cart_session_id_unique').on(table.sessionId).where(sql`${table.sessionId} IS NOT NULL`),
	identity: check(
		'cart_identity_check',
...
export const checkoutDraft = pgTable('checkout_draft', {
	id: text('id').primaryKey(),
	...
	affiliateDisclosureVersion: text('affiliate_disclosure_version').notNull().default('unrecorded'),
	affiliateTermsAcceptanceId: text('affiliate_terms_acceptance_id').references(() => affiliateTermsAcceptance.id),
	status: text('status', {
		enum: ['pending_session', 'checkout_created', 'quarantined', 'paid', 'fulfilled', 'expired', 'failed']
	}).notNull().default('pending_session'),
	currency: varchar('currency', { length: 3 }).notNull(),
	...
	reservationReleasedAt: timestamp('reservation_released_at', { withTimezone: true, mode: 'date' }),
	expiresAt: timestamp('expires_at', { withTimezone: true, mode: 'date' }).notNull(),
	createdAt: timestamp('created_at', { withTimezone: true, mode: 'date' }).notNull().defaultNow(),
	updatedAt: timestamp('updated_at', { withTimezone: true, mode: 'date' }).notNull().defaultNow()
}, (table) => ({
	...
	guestStatusIdx: index('checkout_draft_guest_status_idx').on(table.guestSubjectHash, table.status),
	...
	activeSourceCartIdx: uniqueIndex('checkout_draft_active_source_cart_idx')
		...
		.where(sql`${table.sourceCartId} IS NOT NULL AND ${table.status} IN ('pending_session', 'checkout_created', 'quarantined', 'paid')`),
	buyerIdentity: check(
...
export const checkoutDraftItem = pgTable('checkout_draft_item', {
	id: serial('id').primaryKey(),
	...
	uniqueProductIdx: uniqueIndex('checkout_draft_item_product_idx').on(table.draftId, table.productId),
...
export const checkoutInventoryReservation = pgTable('checkout_inventory_reservation', {
	id: serial('id').primaryKey(),
	draftId: text('draft_id')
		.notNull()
		.references(() => checkoutDraft.id, { onDelete: 'cascade' }),
	...
	draftProductIdx: uniqueIndex('checkout_inventory_reservation_draft_product_idx').on(table.draftId, table.productId),
	statusIdx: index('checkout_inventory_reservation_status_idx').on(table.status),
	quantityPositive: check('checkout_inventory_reservation_quantity_check', sql`${table.quantity} > 0`),
...
export const order = pgTable('order', {
	...
	stripeSessionIdx: uniqueIndex('order_stripe_session_idx').on(table.stripeSessionId).where(sql`${table.stripeSessionId} IS NOT NULL`),
...
export const checkoutPaymentAttempt = pgTable('checkout_payment_attempt', {
	id: text('id').primaryKey(),
	draftId: text('draft_id')
		.notNull()
		.references(() => checkoutDraft.id, { onDelete: 'cascade' }),
	...
	status: text('status', {
		enum: ['pending_session', 'checkout_created', 'quarantined', 'paid', 'expired', 'failed', 'superseded']
	}).notNull().default('pending_session'),
	amountMinor: bigint('amount_minor', { mode: 'number' }).notNull(),
	currency: varchar('currency', { length: 3 }).notNull(),
	...
	draftAttemptIdx: uniqueIndex('checkout_payment_attempt_draft_attempt_idx').on(table.draftId, table.attemptNumber),
	...
	activeDraftIdx: uniqueIndex('checkout_payment_attempt_active_draft_idx')
		.on(table.draftId)
		.where(sql`${table.status} IN ('pending_session', 'checkout_created', 'quarantined', 'paid')`),
	sessionIdx: uniqueIndex('checkout_payment_attempt_stripe_session_idx').on(table.stripeSessionId),
	paymentIntentIdx: uniqueIndex('checkout_payment_attempt_payment_intent_idx').on(table.stripePaymentIntentId),
	attemptNumberPositive: check('checkout_payment_attempt_number_check', sql`${table.attemptNumber} > 0`),
	amountNonnegative: check('checkout_payment_attempt_amount_check', sql`${table.amountMinor} >= 0`),
	safeMinorAmount: check('checkout_payment_attempt_safe_minor_check', sql`${table.amountMinor} <= 9007199254740991`),
	currencyFormat: check('checkout_payment_attempt_currency_check', sql`${table.currency} ~ '^[a-z]{3}$'`)
}));
...
export const stripeWebhookEvent = pgTable('stripe_webhook_event', {
	...
	attemptCountNonnegative: check('stripe_webhook_event_attempt_count_check', sql`${table.attemptCount} >= 0`),
	payloadDigestFormat: check('stripe_webhook_event_payload_digest_check', sql`${table.payloadDigest} ~ '^[0-9a-f]{64}$'`),
	attemptLink: check(
...
export const affiliateCommissionLedger = pgTable('affiliate_commission_ledger', {
	id: text('id').primaryKey(),
	affiliateId: integer('affiliate_id')
		.notNull()
		.references(() => affiliate.id, { onDelete: 'restrict' }),
	...
	termsVersion: text('terms_version').notNull(),
	disclosureVersion: text('disclosure_version').notNull(),
	termsAcceptanceId: text('terms_acceptance_id').references(() => affiliateTermsAcceptance.id, { onDelete: 'restrict' }),
	createdAt: timestamp('created_at', { withTimezone: true, mode: 'date' }).notNull().defaultNow()
}, (table) => ({
	sourceOrderIdx: uniqueIndex('affiliate_commission_ledger_source_order_idx').on(table.sourceOrderId),
	sourceDraftIdx: uniqueIndex('affiliate_commission_ledger_source_draft_idx').on(table.sourceCheckoutDraftId),
	sourceReferenceIdx: uniqueIndex('affiliate_commission_ledger_source_reference_idx').on(table.sourceReference),
	affiliateCreatedIdx: index('affiliate_commission_ledger_affiliate_created_idx').on(table.affiliateId, table.createdAt),
	linkCreatedIdx: index('affiliate_commission_ledger_link_created_idx').on(table.affiliateLinkId, table.createdAt),
	amountPositive: check('affiliate_commission_ledger_amount_check', sql`${table.quotedAmountMinor} > 0 AND ${table.quotedAmountMinor} <= 9007199254740991`),
	rateRange: check('affiliate_commission_ledger_rate_bps_check', sql`${table.commissionRateBps} >= 0 AND ${table.commissionRateBps} <= 10000`),
	currencyFormat: check('affiliate_commission_ledger_currency_check', sql`${table.currency} ~ '^[a-z]{3}$'`),
	tierVersionValid: check('affiliate_commission_ledger_tier_version_check', sql`${table.tierVersion} >= 0`),
	snapshotHashFormat: check('affiliate_commission_ledger_snapshot_hash_check', sql`${table.draftSnapshotHash} ~ '^[0-9a-f]{64}$'`)
}));

// Pending, approval, reversal, payable, and paid records form the immutable commission ledger.
export const affiliateCommissionLedgerEvent = pgTable('affiliate_commission_ledger_event', {
	id: text('id').primaryKey(),
	...
	eventReferenceIdx: uniqueIndex('affiliate_commission_ledger_event_reference_idx').on(table.eventReference),
	...
	commissionCreatedIdx: index('affiliate_commission_ledger_event_commission_created_idx').on(table.commissionId, table.createdAt),
	payoutCreatedIdx: index('affiliate_commission_ledger_event_payout_created_idx').on(table.payoutId, table.createdAt),
	amountRange: check(
	...
	currencyFormat: check('affiliate_commission_ledger_event_currency_check', sql`${table.currency} ~ '^[a-z]{3}$'`),
...
export const guestOrderAccessGrant = pgTable('guest_order_access_grant', {
	id: text('id').primaryKey(),
	draftId: text('draft_id')
		.notNull()
		.references(() => checkoutDraft.id, { onDelete: 'cascade' }),
	tokenHash: text('token_hash').notNull().unique(),
	scope: text('scope', { enum: ['confirmation'] }).notNull().default('confirmation'),
	expiresAt: timestamp('expires_at', { withTimezone: true, mode: 'date' }).notNull(),
	...
	tokenHashFormat: check('guest_order_access_grant_token_hash_check', sql`${table.tokenHash} ~ '^[0-9a-f]{64}$'`)
}));
...
export const file = pgTable('file', {
	...
	entityType: text('entity_type', { enum: ['user', 'product', 'content', 'lms', 'general'] }).notNull().default('general'),
...
export const affiliateAttributionClickRelations = relations(affiliateAttributionClick, ({ one }) => ({
	affiliateLink: one(affiliateLink, {
...
export const affiliateCommissionLedgerRelations = relations(affiliateCommissionLedger, ({ one, many }) => ({
	affiliate: one(affiliate, {
...
export const affiliateCommissionLedgerEventRelations = relations(affiliateCommissionLedgerEvent, ({ one }) => ({
