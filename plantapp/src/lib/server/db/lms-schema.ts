...
export const lmsCourse = pgTable(
	'lms_course',
	{
		...
		slug: text('slug').notNull().unique(),
		description: text('description'),
		courseType: text('course_type', { enum: ['self_paced', 'instructor_led', 'blended', 'cohort'] })
			.notNull()
			.default('self_paced'),
		...
		pricingType: text('pricing_type', { enum: ['free', 'one_time'] })
			.notNull()
			.default('free'),
		enrollmentType: text('enrollment_type', { enum: ['open', 'approval', 'invite', 'capacity'] })
			.notNull()
			.default('open'),
...
// 5. lmsContentBlock
export const lmsContentBlock = pgTable(
	'lms_content_block',
	{
		id: text('id').primaryKey(),
...
// 7. lmsQuestionBank
export const lmsQuestionBank = pgTable(
	'lms_question_bank',
	{
		id: text('id').primaryKey(),
...
// 8. lmsQuestion
export const lmsQuestion = pgTable(
	'lms_question',
	{
		id: text('id').primaryKey(),
		type: text('type', {
...
// 9. lmsQuestionOption
export const lmsQuestionOption = pgTable(
	'lms_question_option',
	{
		id: text('id').primaryKey(),
...
export const lmsEnrollment = pgTable(
	'lms_enrollment',
	{
		...
		courseId: text('course_id')
			.notNull()
			.references(() => lmsCourse.id),
		status: text('status', { enum: ['pending', 'active', 'completed', 'suspended', 'expired'] })
			.notNull()
			.default('active'),
...
export const lmsProgress = pgTable(
	...
	(table) => ({
		...
		enrollmentContentIdx: index('lms_progress_enrollment_content_idx').on(
			table.enrollmentId,
			table.contentBlockId
		),
		enrollmentLessonIdx: index('lms_progress_enrollment_lesson_idx').on(
			table.enrollmentId,
			table.lessonId
		),
		enrollmentContentUniqueIdx: uniqueIndex('lms_progress_enrollment_content_unique_idx').on(
			table.enrollmentId,
			table.contentBlockId
		)
	})
...
// 12. lmsQuizAttempt
export const lmsQuizAttempt = pgTable(
	'lms_quiz_attempt',
	{
		id: text('id').primaryKey(),
		quizId: text('quiz_id')
	...
	(table) => ({
		...
		quizEnrollmentIdx: index('lms_quiz_attempt_quiz_enrollment_idx').on(
			table.quizId,
			table.enrollmentId
		)
	})
...
// 13. lmsQuizAnswer
export const lmsQuizAnswer = pgTable(
	'lms_quiz_answer',
	{
		id: text('id').primaryKey(),
	...
	(table) => ({
		...
		attemptQuestionIdx: uniqueIndex('lms_quiz_answer_attempt_question_idx').on(
			table.attemptId,
			table.questionId
		)
	})
...
export const lmsCertificateTemplate = pgTable(
...
// 17. lmsLearnerBadge
export const lmsLearnerBadge = pgTable(
	'lms_learner_badge',
	{
		id: text('id').primaryKey(),
		userId: text('user_id')
...
export const lmsDiscussionThread = pgTable(
	'lms_discussion_thread',
	{
		id: text('id').primaryKey(),
		courseId: text('course_id')
			.notNull()
	...
	(table) => ({
		...
		coursePinnedIdx: index('lms_discussion_thread_course_pinned_idx').on(
			table.courseId,
			table.isPinned
		)
	})
...
export const lmsCourseCategory = pgTable(
	'lms_course_category',
	{
		id: text('id').primaryKey(),
		name: text('name').notNull(),
...
// 22. lmsCourseTag
export const lmsCourseTag = pgTable(
	'lms_course_tag',
	{
		id: text('id').primaryKey(),
		name: text('name').notNull(),
...
export const lmsCourseToCategoryJoin = pgTable(
	'lms_course_to_category',
	{
		courseId: text('course_id')
			.notNull()
			.references(() => lmsCourse.id, { onDelete: 'cascade' }),
...
export const lmsCourseToTagJoin = pgTable(
	'lms_course_to_tag',
	{
		courseId: text('course_id')
			.notNull()
...
export const lmsCourseProduct = pgTable(
	'lms_course_product',
	{
		id: text('id').primaryKey(),
		courseId: text('course_id')
	...
	(table) => ({
		...
		courseProductIdx: uniqueIndex('lms_course_product_unique_idx').on(
			table.courseId,
...
export const lmsCoursePrerequisite = pgTable(
	'lms_course_prerequisite',
	{
		courseId: text('course_id')
			.notNull()
			.references(() => lmsCourse.id, { onDelete: 'cascade' }),
...
// 27. lmsBookmark
export const lmsBookmark = pgTable(
	'lms_bookmark',
	{
		id: text('id').primaryKey(),
		userId: text('user_id')
