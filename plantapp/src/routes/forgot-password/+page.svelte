		...
		<div class="card-body">
			...
			{#if submitted}
				{#if deliveryAvailable}
					<p role="status">If an eligible account exists, password reset instructions will arrive shortly.</p>
				{:else}
					<p class="text-warning" role="status">Password recovery email is temporarily unavailable. Please contact support.</p>
				{/if}
				<a class="btn btn-primary mt-4" href="/login">Back to login</a>
			{:else}
				<p>Enter your account email address. For security, this confirmation is the same whether or not an account exists.</p>
				<form class="mt-4 space-y-4" onsubmit={submit}>
					<label class="form-control" for="email">
						<span class="label-text">Email address</span>
						<input id="email" class="input input-bordered" type="email" bind:value={email} autocomplete="email" required disabled={loading} />
					</label>
