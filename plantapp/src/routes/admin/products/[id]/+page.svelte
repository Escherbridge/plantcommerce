<script lang="ts">
	...
	const fields = $derived<FormFieldConfig[]>([
	...
	const initialData = $derived<Record<string, unknown>>({
		name: product.name ?? '',
		slug: product.slug ?? '',
		sku: product.sku ?? '',
		price: product.price ?? '',
		...
		stats: product.stats && product.stats.length ? JSON.stringify(product.stats, null, 2) : '',
		specs: product.specs && product.specs.length ? JSON.stringify(product.specs, null, 2) : '',
		faqs: product.faqs && product.faqs.length ? JSON.stringify(product.faqs, null, 2) : '',
	...
	// ── Payload helpers (shared shape with the create page) ──
	...
	async function handleSubmit(formData: Record<string, unknown>) {
		...
		try {
			await trpc.products.updateProduct.mutate({
				id: product.id,
				isActive: formData.isActive !== false,
				...payload
			});
			toasts.addToast({ message: 'Product updated', variant: 'success', duration: 3000 });
			await goto('/admin/products');
		} catch (err) {
			formError = err instanceof Error ? err.message : 'Failed to update product';
			toasts.addToast({ message: formError, variant: 'error', duration: 4000 });
		}
	}
</script>
...
<div class="platform-content">
	<div class="platform-header">
		<div class="product-form-header-row">
			<div>
				<h1 class="platform-header__title">Edit Product</h1>
				<p class="platform-header__subtitle">{product.name}</p>
			</div>
	...
	<div class="platform-card">
		{#key product.id}
			<BaseForm {fields} {initialData} onSubmit={handleSubmit} submitButtonText="Save Changes" />
		{/key}
	</div>
</div>
